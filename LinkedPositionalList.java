...
public class LinkedPositionalList<E> implements PositionalList<E> {
  ...
  public Position<E> addLast(LinkedPositionalList<E> list)
  {
      Node<E> First = list.getFirstNode();
      Node<E> Last = this.getLastNode();
      First.setPrev(Last);
      Last.setNext(First);
      this.trailer=list.trailer;
      list.header.setNext(this.getFirstNode());
      this.size+=list.size();
      return Last.getNext();
      //set the previous pointer from the first element of list to the last element of this.
      //set the next pointer from the last element of this to the first element of list.
      //also set the trailer for this to the the trailer of list.
      //handle floating header for list
  }
  
  //removes and returns a sublist starting from Position p with the specified depth
  private LinkedPositionalList<E> subList(Position<E> p) throws IllegalArgumentException
  {
      Node<E> node= (Node<E>) p;
      LinkedPositionalList<E> subList = new LinkedPositionalList<>();
      subList.header.setNext(node);
      subList.trailer.setPrev(this.trailer.getPrev());
      this.trailer.setPrev(node.getPrev());
      node.getPrev().setNext(this.trailer);
      node.setPrev(subList.header);
      subList.trailer.getPrev().setNext(subList.trailer);
      for(E e:subList)
      {
          subList.size++;
          this.size--;
  ...
  //removes and returns a sublist of the specified depth.
  public LinkedPositionalList<E> subList(int depth) throws IllegalArgumentException
  {
      if(depth>this.size())
      {
          throw new IllegalArgumentException();
      }
      Node<E> node=(Node<E>)this.first();
      for(int i=0;i<this.size()-depth;i++){
          node=node.getNext();
      }
      return this.subList(node);
  }
